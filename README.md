...
* The script runs when you run a search, change the date in an already ran search (using the top bar), or click one of the 7 dates in the fare carousel on a search page. Lastly, if you refresh a page, the script should also run. There are edge cases that will cause the search results to display with the script not running. If this happens, simply refresh the page and the script should run. 
